...
try {
	...
	for(SampleData day_da: day_datas){
		if( count == 3 ){%>
			['小樽', <%=day_da.getTem()%>, <%=day_da.getHum()%>],
		<%}
		else if(count ==5){%>
		['甲府', <%=day_da.getTem()%>, <%=day_da.getHum()%>],
		<%}
		else if(count ==4){%>
		['新潟', <%=day_da.getTem()%>, <%=day_da.getHum()%>],
		<%}
		else if(count ==7){%>
		['那覇', <%=day_da.getTem()%>, <%=day_da.getHum()%>],
		<%}
		else if(count ==6){%>
		['奈良', <%=day_da.getTem()%>, <%=day_da.getHum()%>],
		<%}
		count++;
	}
} finally {
